...
const scorePage = (
  ...
  if (normalizedMetadata.includes(normalizedQuery)) {
    score += 60;
    reasons.push('metadata-phrase');
  }
  tokens.forEach((token) => {
    if (normalizedTitle.includes(token)) score += 10;
    if (normalizedMetadata.includes(token)) score += 8;
...
const getSecondBrainSuggestedCreatePageIds = (
  scope: SecondBrainScope,
): string[] =>
  ...
  if (!isSecondBrainPageAuthorized(pageId, scope, includeGlobalPages)) {
    throw new SecondBrainError(
      ...
      'Second Brain page is outside the active agent scope.',
      {
        ...
        writablePagePrefixes: getSecondBrainScopePrefixes(scope).filter(
          (prefix) => !prefix.endsWith('.md'),
        ),
        suggestedCreatePageIds: getSecondBrainSuggestedCreatePageIds(scope),
      },
    );
  }
};
...
export default class SecondBrainRuntimeService {
  ...
  public async buildContext(
    ...
    const writablePagePrefixes = getSecondBrainScopePrefixes(scope).filter(
      (prefix) => !prefix.endsWith('.md'),
    );
    ...
    const suggestedCreatePageIds = getSecondBrainSuggestedCreatePageIds(
      scope,
    ).filter((pageId) => !existingPageIds.has(pageId));
    const scopeGuide = `### Active writable scope

Scoped page IDs are generated by DBT Studio. Do not invent a friendly scope directory. A suggested create target is authorized but does not exist until \`wiki_update\` creates it.
${existingScopedPageIds.map((pageId) => `- Existing scoped page: \`${pageId}\``).join('\n') || '- Existing scoped pages: none.'}
${suggestedCreatePageIds.map((pageId) => `- Suggested create target (not found until created): \`${pageId}\``).join('\n') || '- Suggested create targets: none.'}
${writablePagePrefixes.map((prefix) => `- Writable prefix: \`${prefix}\``).join('\n')}`;
    const fixedCharacters =
      policy.length + scopeGuide.length + CONTEXT_FOOTER.length + 10;
