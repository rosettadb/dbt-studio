...
import { getActiveNotebookCellEditor } from './notebookCellEditorRegistry';
import {
  NotebookSqlCompletionsContext,
  type NotebookSqlCompletions,
} from './NotebookSqlCompletionsContext';
...
export type NotebookEditorHandle = {
  /**
   * Insert text at the cursor of the most recently focused SQL cell. When no
   * cell has focus yet, a new SQL cell is appended with the text instead.
   * Returns false only when the notebook is not loaded.
   */
  insertText: (text: string) => boolean;
  /** Append a SQL cell with `content`; optionally run it once saved. */
  addSqlCell: (content: string, run?: boolean) => Promise<string | null>;
};
...
>(
  (
    ...
    const [executingCells, setExecutingCells] = useState<Set<string>>(
      new Set(),
    );
    ...
    // Each SQL cell publishes these to the shared `sql` completion provider
    // for its own Monaco model (see SQLCell / lib/monaco/completions/sqlSchema).
    const sqlCompletions = React.useMemo<NotebookSqlCompletions>(
    ...
    // Dropping a table/column from the Data tree onto empty space in the cell
    // list appends a new SQL cell with a SELECT for it. Drops *onto* a cell's
    // editor are claimed by the cell itself (SQLCell → useSchemaObjectDrop) and
    // never reach here; drops on other cell chrome are ignored.
    const handleSchemaDragOver = useCallback((event: React.DragEvent) => {
      if (!hasSchemaDragData(event.dataTransfer)) return;
      if ((event.target as HTMLElement).closest?.('[data-notebook-cell]'))
        return;
      event.preventDefault();
    ...
    const handleSchemaDrop = useCallback(
      (event: React.DragEvent) => {
        ...
        if (
          ...
          content = buildDragSelectStatement(payload);
        } else if (
          payload.kind === 'column' &&
          payload.table &&
          payload.column
        ) {
          content = buildSelectStatement(
            { schema: payload.schema, name: payload.table },
            payload.connectionType,
            { columns: [payload.column] },
          );
        }
        if (content) handleAddCell('sql', content);
      },
      [handleAddCell],
    );
    ...
    // Append a SQL cell and, optionally, run it. Unlike handleAddCell this
    // awaits the save first: the backend writes cell output by cell id, so the
    // cell has to exist on disk before it can be executed.
    const addSqlCell = useCallback(
      async (content: string, run = false): Promise<string | null> => {
        ...
        const newCell: NotebookCellType = {
          ...
          type: 'sql',
          content,
          order: localCellsRef.current.length,
        };
        const updatedCells = [...localCellsRef.current, newCell];
        ...
        if (run) {
          await handleRunCell(newCell.id, content);
        }
        return newCell.id;
      },
      [
    ...
    React.useImperativeHandle(
      ...
      () => ({
        insertText: (text: string) => {
          const editor = getActiveNotebookCellEditor(notebookId);
          if (editor && insertTextAtCursor(editor, text)) return true;
      ...
      [notebookId, notebook, addSqlCell],
    );
    ...
    return (
      <NotebookSqlCompletionsContext.Provider value={sqlCompletions}>
        <Box
          sx={{
            flex: 1,
            minHeight: 0,
        ...
        >
          ...
          >
            ...
            <DialogContent>
              <DialogContentText>
                This will permanently delete all {localCells.length} cells in
      ...
      </NotebookSqlCompletionsContext.Provider>
    );
  },
);
