...
import { ConnectionInput, Project, Table } from '../../../types/backend';
 ...
 * menu and drag-and-drop to act on the editor without owning its state.
 */
export type SqlEditorHandle = {
  /** Insert text at the cursor. Returns false when no editor is mounted. */
  insertText: (text: string) => boolean;
  /** Run `sql` as-is and show the results; the editor content is untouched. */
  runQuery: (sql: string) => void;
};

type Props = {
  completions: Omit<monacoType.languages.CompletionItem, 'range'>[];
  /** Raw table list behind `completions`; enables context-aware completion. */
  schemaTables?: Table[];
...
export const SqlEditor = React.forwardRef<SqlEditorHandle, Props>(
  (
    {
      completions,
      schemaTables,
    ...
    React.useImperativeHandle(
      ...
      () => ({
        ...
        runQuery: (sql: string) => {
          if (sql.trim()) handleRunQuery(sql);
        },
      }),
      // handleRunQuery is recreated every render; refresh the handle with it.
