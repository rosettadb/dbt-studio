...
describe('ConnectorsService (main)', () => {
  ...
  describe('parseProjectConnectionFiles (kinetica)', () => {
    ...
    it('round-trips a profile generated by the Studio via the db-url env var', async () => {
      const os = jest.requireActual('os');
      const path = jest.requireActual('path');
      const envKey = 'db-url-kin_rt';
      const previousEnv = process.env[envKey];
      const projectPath = await fs.promises.mkdtemp(
        path.join(os.tmpdir(), 'rosetta-kinetica-roundtrip-'),
      );
      ...
      // generateProfilesYml writes `host: '{{ env_var("db-url-kin_rt") }}'`;
      // the Studio exports the pre-built URL under that name at runtime.
      process.env[envKey] =
        'https://tenant.kinetica.com:8082/cluster-1/gpudb-0';
